...
public final class TKRecoveryPhraseItemView: UIView, ConfigurableView {
    ...
    override public func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        return CGSize(width: size.width, height: textStyles.wordTextStyle.lineHeight)
    }
    ...
    private func getTextStylesToFit(
        word: String,
        wordWidth: CGFloat,
        height: CGFloat
    ) -> (wordTextStyle: TKTextStyle, indexTextStyle: TKTextStyle) {
        ...
        return textStyles.first(where: { isTextStyleFit($0.0) }) ?? (.body3, .body4)
    }
