...
public enum TonPayloadFormat {
    ...
    public enum JettonBurnCustomPayload {
        ...
        public static func checkAndConvertPayload(_ customPayload: inout JettonBurnCustomPayload?) throws {
            if let payload = customPayload,
               case let .cellPayload(cell) = payload,
               cell.bits.length == 20,
               cell.refs.isEmpty
            {
    ...
    public static func from(cell: Cell) throws -> TonPayloadFormat {
        ...
        do {
            ...
            case 0x0F8A_7EA5:
                let queryId = try normalizeQueryId(slice.loadUint(bits: 64))
            ...
            case 0x595F_07BC:
                ...
                var customPayload: JettonBurnCustomPayload? = try slice.loadBoolean() ? .cellPayload(slice.loadRef()) : nil
                try slice.endParse()
            ...
            case 0x47D5_4391:
                let queryId = try normalizeQueryId(slice.loadUint(bits: 64))
                let appId = try slice.remainingBits > 0 ? BigUInt(slice.loadUint(bits: 64)) : nil
                try slice.endParse()
            ...
            case 0x69FB_306C:
                ...
                let expirationDate = try BigUInt(slice.loadUint(bits: 48))
