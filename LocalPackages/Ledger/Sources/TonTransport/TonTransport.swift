...
public enum TonTransportError: Swift.Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidPayload:
            "Invalid payload"
        case let .packageTooLarge(length):
...
public class TonTransport {
    ...
    public func isAppOpen() async throws -> (Bool, String) {
        ...
        case "BOLOS":
            do {
                _ = try await doRequest(ins: TonTransport.INS_OPEN_APP, p1: 0x00, p2: 0x00, data: Data("TON".utf8))
            } catch {}
            return (false, "")
        default:
            return (false, "")
    ...
    public static func buildTransfer(transaction: Transaction) throws -> LedgerTransfer {
        ...
        case let .jettonTransfer(jettonPayload):
            hints = putUint8(1) + putUint32(0x01)
            let builder = try Builder().store(uint: 0x0F8A_7EA5, bits: 32)
            var bytes = [UInt8]()

            if let queryId = jettonPayload.queryId {
                try bytes += putUint8(1) + putUint64(queryId)
        ...
        case let .nftTransfer(nftPayload):
            hints = putUint8(1) + putUint32(0x02)
            let builder = try Builder().store(uint: 0x5FCC_3D14, bits: 32)
            var bytes = [UInt8]()
        ...
        // Jetton Burn
        case let .jettonBurn(burnPayload):
            hints = putUint8(1) + putUint32(0x03)
            let builder = try Builder().store(uint: 0x595F_07BC, bits: 32)
            var bytes = [UInt8]()
            ...
            if let customPayload = burnPayload.customPayload {
                ...
                case let .cellPayload(cell):
                    bytes += putUint8(1) + putCellRef(cell)
                    try builder.storeMaybe(ref: cell)
                case let .byteArrayPayload(customPayloadBytes):
                    bytes += putUint8(2) + putUint8(UInt8(customPayloadBytes.count)) + customPayloadBytes
                    try builder.storeMaybe(ref: Builder().store(data: customPayloadBytes).endCell())
                }
            } else {
                bytes += putUint8(0)
                try builder.store(bit: false)
        ...
        // Add Whitelist
        case let .addWhitelist(whitelistPayload):
            hints = putUint8(1) + putUint32(0x04)
            let builder = try Builder().store(uint: 0x7258_A69B, bits: 32)
            var bytes = [UInt8]()
        ...
        case let .changeDNSRecord(dnsPayload):
            hints = putUint8(1) + putUint32(0x09)
            let builder = try Builder().store(uint: 0x4EB1_F0F9, bits: 32)
            var bytes = [UInt8]()
        ...
        case let .singleNominatorChangeValidator(nominatorPayload):
        ...
        case let .singleNominatorWithdraw(withdrawPayload):
        ...
        case let .tokenBridgePaySwap(swapPayload):
        ...
        // Tonstakers Deposit
        case let .tonstakersDeposit(depositPayload):
            hints = putUint8(1) + putUint32(0x07)
            let builder = try Builder().store(uint: 0x47D5_4391, bits: 32)
            var bytes = [UInt8]()
            ...
            if let appId = depositPayload.appId {
                try bytes += putUint8(1) + putUint64(appId)
        ...
        // Vote for Proposal
        case let .voteForProposal(votePayload):
            hints = putUint8(1) + putUint32(0x08)
            let builder = try Builder().store(uint: 0x69FB_306C, bits: 32)
            var bytes = [UInt8]()
            ...
            try bytes += putAddress(votePayload.votingAddress) + putUint48(votePayload.expirationDate) + putUint8(votePayload.vote ? 1 : 0) + putUint8(votePayload.needConfirmation ? 1 : 0)
        ...
        let transfer = try Builder()
            .store(uint: 698_983_191, bits: 32)
    ...
    public func signAddressProof(
        ...
        let timestampData = try Data(TonTransport.putUint64(BigUInt(timestamp)))
        let pkg = path.data
