...
private var kObserveTokensHandle: UInt8 = 0

public extension KeyboardObserving where Self: UIViewController {
    private var observeTokens: [NSObjectProtocol]? {
        get {
            return objc_getAssociatedObject(self, &kObserveTokensHandle) as? [NSObject]
        }
        set {
            objc_setAssociatedObject(self, &kObserveTokensHandle, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }
