...
public final class TKListItemTitleView: UIView {
    ...
    override public func layoutSubviews() {
        ...
        let titleFrame = CGRect(
            origin: CGPoint(x: 0, y: bounds.height / 2 - layoutSize.titleSize.height / 2),
            size: layoutSize.titleSize
        )
        let captionFrame = CGRect(
            origin: CGPoint(x: titleFrame.maxX + .captionPadding, y: bounds.height / 2 - layoutSize.captionSize.height / 2),
            size: layoutSize.captionSize
        )

        var tagOriginX = captionFrame.maxX
        for (index, view) in tagViews.enumerated() {
            guard layoutSize.tagSizes.count > index else { continue }
            let size = layoutSize.tagSizes[index]
            let frame = CGRect(
                origin: CGPoint(x: tagOriginX, y: bounds.height / 2 - size.height / 2),
                size: size
            )
            view.frame = frame
        ...
        let iconFrame = CGRect(
            origin: CGPoint(x: tagOriginX + .iconLeftPadding, y: bounds.height / 2 - layoutSize.iconSize.height / 2),
    ...
    private func calculateLayoutSize(size: CGSize) -> LayoutSize {
        ...
        let titleSize: CGSize = {
            ...
            let sizeThatFits = titleLabel.sizeThatFits(CGSize(
                width: width,
                height: 0
            ))
...
private extension CGFloat {
    ...
    static let captionPadding: CGFloat = 6
}
