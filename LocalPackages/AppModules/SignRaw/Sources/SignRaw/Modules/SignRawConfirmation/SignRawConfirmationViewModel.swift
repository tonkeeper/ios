...
public struct SignRawSendAnalyticsPayload {
    ...
    public init(assetNetwork: String, tokenSymbol: String, amount: Double, feePaidIn: FeePaidIn) {
...
public protocol SignRawConfirmationModuleOutput: AnyObject {
    ...
    var didRequestSendConfirm: ((SignRawSendAnalyticsPayload) -> Void)? { get set }
...
public final class SignRawConfirmationViewModelImplementation: SignRawConfirmationViewModel, SignRawConfirmationModuleOutput, SignRawConfirmationModuleInput {
    ...
    public var didRequestSendConfirm: ((SignRawSendAnalyticsPayload) -> Void)?
    ...
    private func emulate() {
        Task { [weak self] in
            ...
            do {
                ...
                if let payload = makeSendAnalyticsPayload(
                    emulation: emulationResult,
                    transferType: emulationResult.transferType
                ) {
    ...
    private func confirmTransaction() {
        Task {
            if case let .success(_, emulation, transferType) = state.emulationState,
               let payload = makeSendAnalyticsPayload(emulation: emulation, transferType: transferType)
            {
                didRequestSendConfirm?(payload)
            }
    ...
    private func makeSendAnalyticsPayload(
        ...
        case let .tonTransfer(transfer):
            ...
            return SignRawSendAnalyticsPayload(
                assetNetwork: "ton",
