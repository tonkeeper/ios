...
public enum TonTron {
    ...
    public static func keyPair(_ tonMnemonic: [String]) -> TronSwift.KeyPair {
        let tronMnemonic = tonMnemonicToTronMnemonic(tonMnemonic)
        return HDKeys.keyPair(mnemonic: tronMnemonic, derivationCurve: Secp256k1DerivationCurve())
    }

    public static func derivedKeyPair(_ tonMnemonic: [String], index: Int) throws -> TronSwift.KeyPair {
        let tronMnemonic = tonMnemonicToTronMnemonic(tonMnemonic)
