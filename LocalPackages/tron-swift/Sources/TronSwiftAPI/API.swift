...
public struct API {
  ...
  public func tronUSDTBalance(owner: Address, network: Network) async throws -> BigUInt {
    let contractAddress = USDT.address
    let request = ContractCallRequest.request(
      contractAddress: contractAddress,
      data: BalanceOfMethod(owner: owner).encode()
    )
    let result = try await performDataRequest(request: request, network: network)
  ...
  public func getTronHistory(address: Address,
                             limit: Int,
                             minTimestamp: Int64?,
                             maxTimestamp: Int64?,
                             fingerprint: String?,
                             network: Network) async throws -> TransactionsResponse {
    let path = network.networkURL.appendingPathComponent("v1/accounts/\(address.base58)/transactions/trc20")
  ...
  public func estimateUSDTResources(owner: Address,
                                    method: ContractMethod,
                                    network: Network) async throws -> (energy: Int, bandwidth: Int) {
    ...
    guard let result = (json["result"] as? [String: Any])?["result"] as? Bool,
          result,
          let energyUsed = json["energy_used"] as? Int else {
  ...
  public func getTransferTransaction(owner: Address,
                                     method: ContractMethod,
                                     feeLimit: Int,
                                     network: Network) async throws -> Transaction {
    let json = try await triggerSmartContract(network: network,
                                   owner: owner,
                                   contract: USDT.address,
                                   method: method,
                                   feeLimit: feeLimit)
    guard let transactionJson = json["transaction"] as? [String: Any],
    let transaction = Transaction(json: transactionJson) else {
      throw NSError(domain: "", code: 1)
    }
    return transaction
  }
  
  public func getSignWeightTransaction(network: Network,
                                       transaction: Transaction) async throws -> Transaction {
    let json = try await getSignWeight(network: network, transaction: transaction)
  ...
  public func getSignWeight(network: Network,
                            transaction: Transaction) async throws -> [String: Any] {
    let path = network.networkURL.appendingPathComponent("wallet/getsignweight")
  ...
  public func triggerConstantContract(network: Network,
                                      owner: Address,
                                      contract: Address,
                                      method: ContractMethod,
                                      visible: Bool) async throws -> [String: Any] {
    let path = network.networkURL.appendingPathComponent("wallet/triggerconstantcontract")
    var urlRequest = URLRequest(url: path)
    urlRequest.httpMethod = "POST"
  ...
  public func triggerSmartContract(network: Network,
                                   owner: Address,
                                   contract: Address,
                                   method: ContractMethod,
                                   feeLimit: Int) async throws -> [String: Any] {
    let path = network.networkURL.appendingPathComponent("wallet/triggersmartcontract")
    var urlRequest = URLRequest(url: path)
    urlRequest.httpMethod = "POST"
    urlRequest.httpBody = try JSONSerialization.data(withJSONObject: [
  ...
  public func performDataRequest(request: JSONRpcRequest, network: Network) async throws -> Data {
    try await perform(request: request, network: network) { rpcResponse in
      guard let hexString = rpcResponse.result as? String,
            let value = Data(hex: hexString) else {
  ...
  public func perform<T>(request: JSONRpcRequest, network: Network, responseParse: (_ rpcResponse: JSONRpcResponse.SuccessResponse) throws -> T) async throws -> T {
    let path = network.networkURL.appendingPathComponent("jsonrpc")
    var urlRequest = URLRequest(url: path)
    urlRequest.httpMethod = "POST"
