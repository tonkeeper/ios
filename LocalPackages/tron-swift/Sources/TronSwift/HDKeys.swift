...
public enum HDKeys {
  public static func keyPair(mnemonic: [String],
                             derivationCurve: DerivationCurve) -> KeyPair {
    let seed = Mnemonic.mnemonicToSeed(mnemonic: mnemonic)
    return keyPair(seed: seed, derivationCurve: derivationCurve)
  }
  
  public static func derivedKeyPair(mnemonic: [String],
                                    path: String,
                                    derivationCurve: DerivationCurve) throws -> KeyPair {
    let seed = Mnemonic.mnemonicToSeed(mnemonic: mnemonic)
  ...
  public static func keyPair(seed: Data,
                             derivationCurve: DerivationCurve) -> KeyPair {
    let privateKey = privateKey(seed: seed, derivationCurve: derivationCurve)
    return KeyPair(publicKey: PublicKey(data: Data()), privateKey: privateKey)
  }
  
  private static func privateKey(seed: Data,
                                 derivationCurve: DerivationCurve) -> PrivateKey {
  ...
  private static func derivedPrivateKey(seed: Data,
                                        path: String,
                                        derivationCurve: DerivationCurve) throws -> PrivateKey {
    let privateKey = privateKey(seed: seed, derivationCurve: derivationCurve)
    var path = path
