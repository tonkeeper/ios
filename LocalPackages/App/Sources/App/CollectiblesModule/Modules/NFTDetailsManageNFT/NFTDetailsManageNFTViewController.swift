...
final class NFTDetailsManageNFTViewController: UIViewController, NFTDetailsManageNFTOutput {
    ...
    func getState() -> NFTDetailsManageNFTState {
        guard wallet.isReportSpamAvailable,
              nft.isUnverified,
              nftManagementStore.state.nftStates[getNftManagementItem()] != .approved
        else {
            return .hidden
        }
    ...
    private func approveNFT() {
        ...
        let task = Task { @MainActor [weak self] in
            ...
            Task { [weak self] in
                guard let self else { return }
                do {
                    try await nftService.changeSuspiciousState(nft, network: wallet.network, isScam: false)
    ...
    private func markSpamNFT() {
        ...
        let task = Task { @MainActor [weak self] in
            ...
            Task { [weak self] in
                guard let self else { return }
                do {
                    try await nftService.changeSuspiciousState(nft, network: wallet.network, isScam: true)
