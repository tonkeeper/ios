...
final class SettingsPurchasesViewController: GenericViewViewController<SettingsPurchasesView> {
    ...
    typealias RegularItemConfiguration = UICollectionView.CellRegistration<
        SettingsPurchasesItemCell,
        String
    >
    typealias ButtonCellConfiguration = UICollectionView.CellRegistration<
        TKButtonCell,
        TKButtonCell.Model
    >
...
private extension SettingsPurchasesViewController {
    ...
    func createDataSource() -> DataSource {
        ...
        let headerRegistration = HeaderRegistration(elementKind: .sectionHeaderIdentifier) { [weak self] supplementaryView, _, indexPath in
            guard let self else { return }
            let section = self.dataSource.snapshot().sectionIdentifiers[indexPath.section]
            supplementaryView.configure(model: TKListTitleView.Model(title: section.title, textStyle: .h3))
        }

        let sectionFooterRegistration = SectionFooterRegistration(elementKind: .sectionFooterIdentifier) { [weak self] supplementaryView, _, indexPath in
            guard let self else { return }
            let section = self.dataSource.snapshot().sectionIdentifiers[indexPath.section]
    ...
    func createLayout() -> UICollectionViewCompositionalLayout {
        ...
        return UICollectionViewCompositionalLayout(sectionProvider: { [weak viewModel, dataSource] sectionIndex, _ in
