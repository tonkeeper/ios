...
final class HistoryCellActionView: UIControl, ConfigurableView, ReusableView {
    ...
    let progressLoaderView = HistoryCellLoaderView()
    ...
    struct Model {
        ...
        init(
            ...
            loaderState: HistoryCellLoaderView.State = .idle
        ) {
    ...
    func configure(model: Model) {
        ...
        let isProgressHidden = model.contentConfiguration.iconViewConfiguration == nil
        if !isProgressHidden {
            switch model.loaderState {
            case .idle:
                progressLoaderView.isHidden = true
                progressLoaderView.updateState(.idle, animated: false)
            case let .progress(value):
                progressLoaderView.isHidden = value <= 0
                if value > 0 {
                    progressLoaderView.updateState(.idle, animated: false)
                    progressLoaderView.updateState(.progress(value), animated: true)
                } else {
                    progressLoaderView.updateState(.idle, animated: false)
                }
            case .infinite:
                progressLoaderView.isHidden = false
                progressLoaderView.updateState(.infinite, animated: true)
            }
        } else {
            progressLoaderView.isHidden = true
            progressLoaderView.updateState(.idle, animated: false)
    ...
    override func layoutSubviews() {
        ...
        if !contentView.iconView.isHidden {
            let iconImageView = contentView.iconView.imageView
            let iconImageViewCenter = iconImageView.convert(
                CGPoint(x: iconImageView.bounds.midX, y: iconImageView.bounds.midY),
                to: contentContainer
            )
            progressLoaderView.center = iconImageViewCenter
        } else {
            progressLoaderView.frame = .zero
        }
    }
...
private extension HistoryCellActionView {
    func setup() {
        ...
        contentContainer.addSubview(progressLoaderView)
