...
final class BuyViewModelImplementation: BuySellViewModel, BuySellModuleOutput, BuySellModuleInput {
    ...
    func viewDidLoad() {
        buyInputController.didUpdateModel = { [weak self] model in
            DispatchQueue.main.async {
                self?.didUpdateInput?(Self.createInputModel(model))
            }
        }
        
        buyInputController.shouldUpdateSendTokenInput = { [weak self] text in
            DispatchQueue.main.async {
        ...
        buyInputController.start()
    }
    
    func didEditInput(_ input: String?) {
        buyInputController.setSendInput(input ?? "0")
    }
    
    func toggleInputMode() {
        buyInputController.swapTokens()
    }
    ...
    init(buyInputController: BuyInputController) {
        self.buyInputController = buyInputController
    }
}

private extension BuyViewModelImplementation {
    static func createInputModel(_ model: BuyInputController.Model) -> BuySellInputModel {
