...
final class BatteryRechargeViewController: GenericViewViewController<BatteryRechargeView>, KeyboardObserving {
    ...
    init(
        viewModel: BatteryRechargeViewModel,
...
private extension BatteryRechargeViewController {
    ...
    func createDataSource() -> BatteryRecharge.DataSource {
        ...
        return BatteryRecharge.DataSource(
            collectionView: customView.collectionView
        ) { [weak self]
            collectionView, indexPath, itemIdentifier -> UICollectionViewCell? in
                guard let self else { return nil }
                switch itemIdentifier {
                case let .rechargeOption(item):
                    let cell = collectionView.dequeueConfiguredReusableCell(
                        using: listCellRegistration,
                        for: indexPath,
                        item: item.listCellConfiguration
                    )
                    cell.defaultAccessoryViews = [createRadioButtonAccessoryView(
                        isEnable: item.isEnable,
                        isSelected: false
                    )]
                    cell.selectionAccessoryViews = [createRadioButtonAccessoryView(
                        isEnable: item.isEnable,
                        isSelected: true
                    )]
