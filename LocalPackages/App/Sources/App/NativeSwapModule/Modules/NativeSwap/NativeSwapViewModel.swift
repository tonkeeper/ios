...
final class NativeSwapViewModelImplementation: NativeSwapViewModel, NativeSwapModuleOutput {
    ...
    private let swapConfirmationSubject = PassthroughSubject<(SwapConfirmationData, UUID, KeeperCore.Token, KeeperCore.Token), Never>()
    ...
    func handleContinueTap() {
        ...
        let sendSymbol = model.fromToken == .usdtTron ? TronSwift.USDT.name : model.fromToken.symbol
        let sendFormatted = "\(sendInput) \(sendSymbol)"

        let receiveSymbol = model.toToken == .usdtTron ? TronSwift.USDT.name : model.toToken.symbol
    ...
    private func setupBinding() {
        ...
        swapConfirmationSubject
            ...
            .sink { [weak self] data, requestID, fromToken, toToken in
                guard let self else { return }
                ...
                streamTask = Task { [weak self] in
                    ...
                    await self?.subscribeToSwapStream(data: data, requestID: requestID, fromToken: fromToken, toToken: toToken)
                }
            }
            .store(in: &subscriptions)
    }
    ...
    private func requestInitialRates() {
        ...
        Task { [weak self] in
            ...
            async let rateABTask: Void = {
                do {
                    for try await confirmation in await self.nativeSwapService.subscribeToSwapConfirmation(data: dataAB, network: self.wallet.network) {
            ...
            async let rateBATask: Void = {
                do {
                    for try await confirmation in await self.nativeSwapService
    ...
    private func updateStreamConfirmation() {
        ...
        // Capture current token pair snapshot for validation
        swapConfirmationSubject.send((data, currentRequestID, model.fromToken, model.toToken))
    }
    ...
    private func subscribeToSwapStream(
        ...
        do {
            for try await confirmation in nativeSwapService.subscribeToSwapConfirmation(data: data, network: wallet.network) {
                ...
                await handleSwapEvent(confirmation, requestID: requestID, fromToken: fromToken, toToken: toToken)
    ...
    private func handleSwapEvent(
        ...
        guard
            ...
            model.fromToken == fromToken,
            model.toToken == toToken
        else {
            return
        }
...
extension NativeSwapViewModelImplementation {
    final class RateFormatter {
        ...
        func getToTokenSymbol(token: KeeperCore.Token) -> String {
            ...
            case .usdtTron: token.symbol + " TRC20"
            }
        }
    }
}
