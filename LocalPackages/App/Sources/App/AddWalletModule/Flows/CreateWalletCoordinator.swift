...
private extension CreateWalletCoordinator {
    func openCustomizeWalletSkippingNotifications(
        router: NavigationControllerRouter,
        passcode: String,
        phrase: [String],
        backupDate: Date?
    ) {
        var viewControllers = router.rootViewController.viewControllers
        if viewControllers.last is OnboardingInfoViewController {
