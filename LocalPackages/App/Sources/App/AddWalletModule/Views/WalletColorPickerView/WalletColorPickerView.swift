...
final class WalletColorPickerView: UIView, ConfigurableView {
    lazy var collectionView: UICollectionView = TKUICollectionView(frame: .zero, collectionViewLayout: createLayout())

    var model = Model(items: [], intitialSelectedIndex: nil)

    private let cellConfig = UICollectionView.CellRegistration<WalletColorPickerViewCell, Model.ColorItem> { cell, _, itemIdentifier in
