...
struct StakingOptionListItemMapper {
  ...
  func mapOptionItem(
    _ item: StakingOptionItem,
    accessoryClosure: @escaping ((String) -> Void),
    selectionClosure: @escaping (() -> Void)
    ...
    switch item.image {
    case .fromResource:
      imageConfiguration = .image(item.kind.image)
    case .url(let url):
    ...
    let listItemConfiguration = TKUIListItemView.Configuration(
      ...
      accessoryConfiguration: createAccessoryConfiguration(item: item, accessoryClosure: accessoryClosure)
    )
    
    return .init(id: item.id, listItemConfiguration: listItemConfiguration, selectionClosure: selectionClosure)
  }
}
