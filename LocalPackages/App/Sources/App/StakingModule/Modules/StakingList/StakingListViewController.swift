...
private extension StakingListViewController {
    ...
    func createLayout() -> UICollectionViewCompositionalLayout {
        ...
        return UICollectionViewCompositionalLayout { [dataSource] sectionIndex, _ in
            let snapshot = dataSource.snapshot()
            let snapshotSection = snapshot.sectionIdentifiers[sectionIndex]

            let itemLayoutSize = NSCollectionLayoutSize(
                widthDimension: .fractionalWidth(1.0),
                heightDimension: .estimated(.itemHeigth)
            )
    ...
    func createDataSource() -> StakingList.DataSource {
        let itemCellConfiguration = ListItemCellRegistration.registration(collectionView: customView.collectionView)
        let dataSource = StakingList.DataSource(collectionView: customView.collectionView) { collectionView, indexPath, itemIdentifier in
            let cell = collectionView.dequeueConfiguredReusableCell(
                using: itemCellConfiguration,
                for: indexPath,
                item: itemIdentifier.configuration
            )
            cell.defaultAccessoryViews = [itemIdentifier.accessory].compactMap { $0?.view }
            return cell
        }

        let sectionHeaderRegistration = SectionHeaderRegistration(elementKind: .sectionHeaderElementKind) { [weak self] supplementaryView, _, indexPath in
