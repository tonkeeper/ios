...
struct StakingLinksViewModelBuilder {
    func buildModel(
        ...
        openURLInApp: @escaping (URL) -> Void
    ) -> StakingDetailsLinksView.Model {
    ...
    func buildModelEthena(
        links: [String],
        openURLInApp: @escaping (URL) -> Void
    ) -> StakingDetailsLinksView.Model {
        let linkItems = links.compactMap { link -> StakingDetailsLinksView.Model.LinkItem? in
            ...
            return StakingDetailsLinksView.Model.LinkItem(title: host, icon: .TKUIKit.Icons.Size16.globe) {
                openURLInApp(url)
            }
        }
