...
final class TokenDetailsViewController: GenericViewViewController<TokenDetailsView> {
    ...
    init(
        viewModel: TokenDetailsViewModel,
...
private extension TokenDetailsViewController {
    func setupBindings() {
        ...
        viewModel.didUpdateBannerItems = { [weak self] bannerItems in
            ...
            (self.listContentViewController.scrollView as? UICollectionView)?.reloadData()
        }
    }
    ...
    func setupListContent() {
        ...
        listContentViewController.didPullToRefresh = { [weak self] in
