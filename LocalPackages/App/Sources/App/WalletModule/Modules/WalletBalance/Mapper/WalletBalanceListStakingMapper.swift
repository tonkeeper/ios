...
struct WalletBalanceListStakingMapper {
    ...
    private func mapStakingItemComment(
        _ item: ProcessedBalanceStakingItem,
        isSecure: Bool,
        isStakingEnable: Bool,
        stakingCollectHandler: (() -> Void)?
            ...
            let amount: String = {
                if isSecure {
                    ...
                    return amountFormatter.format(
                        amount: BigUInt(item.info.pendingDeposit),
        ...
        if item.info.pendingWithdraw > 0 {
            let amount: String = {
                if isSecure {
                    ...
                    return amountFormatter.format(
                        amount: BigUInt(item.info.pendingWithdraw),
                        fractionDigits: TonInfo.fractionDigits
                    )
                }
            }()
        ...
        if item.info.readyWithdraw > 0 {
            let amount: String = {
                if isSecure {
                    ...
                    return amountFormatter.format(
                        amount: BigUInt(item.info.readyWithdraw),
                        fractionDigits: TonInfo.fractionDigits
                    )
                }
            }()
