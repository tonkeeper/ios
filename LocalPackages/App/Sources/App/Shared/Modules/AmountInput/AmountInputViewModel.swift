...
protocol AmountInputModuleInput: AnyObject {
    ...
    var isBalanceVisible: Bool { get set }
    var isCurrencySwitchEnabled: Bool { get set }
    var isConvertedAmountHidden: Bool { get set }
    var isConvertedShimmering: Bool { get set }
...
protocol AmountInputViewModel: AnyObject {
    ...
    var didUpdateBalanceIsVisible: ((Bool) -> Void)? { get set }
...
final class AmountInputViewModelImplementation: AmountInputViewModel, AmountInputModuleOutput, AmountInputModuleInput {
    ...
    var didUpdateBalanceIsVisible: ((Bool) -> Void)?
    ...
    var isBalanceVisible: Bool = true {
        didSet {
            didUpdateBalanceVisible()
        }
    }

    var isCurrencySwitchEnabled: Bool = true {
        didSet {
            updateValueView()
        }
    }

    var isConvertedAmountHidden: Bool = false {
        didSet {
            updateValueView()
        }
    }

    var isConvertedShimmering: Bool = false {
        didSet {
            updateValueView()
        }
    }
    ...
    private func didUpdateBalanceVisible() {
        didUpdateBalanceIsVisible?(isBalanceVisible)
    }
    ...
    private func updateValueView() {
        let configuration = AmountInputValueView.Configuration(
            ...
            convertedButtonConfiguration: AmountInputConvertedButton.Configuration(
                ...
                showSwitchIcon: isCurrencySwitchEnabled,
                showConvertedAmount: !isConvertedAmountHidden,
                isUserInteractionEnabled: isCurrencySwitchEnabled,
    ...
    private func updateBalanceView() {
        ...
        if sourceAmount.isZero {
            let formattedBalance = amountFormatter.format(
                ...
                fractionDigits: sourceUnit.fractionalDigits,
                accessory: .symbol(sourceUnit.symbol)
            )
    ...
    func inputStringToAmount(input: String) -> BigUInt {
        ...
        return BigUInt(stringLiteral: components.joined() + zeroString)
    }
}
