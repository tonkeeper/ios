...
enum WalletTransferSignError: Error {
...
private extension WalletTransferSignCoordinator {
            ...
            onInput: { [weak self, wallet, keeperCoreMainAssembly, transferData] passcode in
                ...
                Task {
                    do {
                        ...
                        self.didFail?(
                            .failedToSign(
                                message: "passcode input failure: \(error.localizedDescription)"
                            )
                        )
                    }
                }
    ...
    func handleSignerSign(
        ...
        await withCheckedContinuation { continuation in
            DispatchQueue.main.async { [wallet] in
                guard let url = try? self.createTonSignURL(
                    transfer: walletTransfer.signingMessage.endCell().toBoc(),
                    publicKey: publicKey,
                    revision: revision,
                    network: network,
                    isOnDevice: false
    ...
    func handleKeystoneSign(
        ...
        await withCheckedContinuation { continuation in
            DispatchQueue.main.async { [wallet] in
                guard let ur = try? self.createKeystoneSignUR(
                    transfer: walletTransfer.signingMessage.endCell().toBoc(),
                    path: path,
                    xfp: xfp,
                    publicKey: publicKey,
    ...
    func handleSignerSignOnDevice(
        ...
        await withCheckedContinuation { continuation in
            guard let url = try? createTonSignURL(
                transfer: walletTransfer.signingMessage.endCell().toBoc(),
                publicKey: publicKey,
                revision: revision,
                network: network,
    ...
    func handleLedgerConfirmError(_ error: LedgerConfirmError) {
        switch error {
        case let .versionTooLow(_, requiredVersion):
