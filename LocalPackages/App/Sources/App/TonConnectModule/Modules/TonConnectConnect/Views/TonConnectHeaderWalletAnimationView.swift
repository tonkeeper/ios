...
final class TonConnectHeaderWalletAnimationView: UIView, ConfigurableView {
    ...
    private var animators = [ObjectIdentifier: UIViewPropertyAnimator]()
    ...
    override func layoutSubviews() {
        ...
        separatorView.frame = .init(
            origin: .init(x: bounds.width / 2 - .separatorWidth / 2, y: bounds.height / 2 - .separatorHeigth / 2),
...
private extension TonConnectHeaderWalletAnimationView {
    ...
    func animateFirstLabel(label: UILabel, container: UIView) {
        ...
        animator.addCompletion { [weak self] position in
            ...
            self.removeAnimator(for: label)
            guard position == .end, self.window != nil else { return }
            self.animateSecondLabel(label: label, container: container)
        }
        setAnimator(animator, for: label)
    ...
    func setAnimator(_ animator: UIViewPropertyAnimator, for label: UILabel) {
        let key = ObjectIdentifier(label)
        animators[key]?.stopAnimation(true)
        animators[key] = animator
    }

    func removeAnimator(for label: UILabel) {
        animators[ObjectIdentifier(label)] = nil
    }
}
