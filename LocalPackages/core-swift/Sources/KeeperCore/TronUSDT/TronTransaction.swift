...
public struct TronTransaction: Codable, Hashable, Equatable {
...
extension TronTransaction {
  init(apiTransaction: TronTransactionsListTransactionsInner) throws {
    self.txID = apiTransaction.txid
    self.timestamp = Int64(apiTransaction.timestamp)
    self.fromAccount = try TronSwift.Address(address: apiTransaction.fromAccount)
    self.toAccount = try TronSwift.Address(address: apiTransaction.toAccount)
    self.amount = BigUInt(apiTransaction.amount) ?? 0
    self.isPending = apiTransaction.isPending
    self.isFailed = apiTransaction.isFailed
    self.batteryCharges = apiTransaction.batteryCharges
  }
}
