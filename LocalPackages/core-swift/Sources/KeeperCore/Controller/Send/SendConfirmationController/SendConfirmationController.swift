...
private extension SendConfirmationController {
    ...
    func buildEmulatedModel(fee: Int64?) async -> SendConfirmationModel {
        ...
        if let fee = fee {
            ...
            if let rates = rates.tonRates.first(where: { $0.currency == currency }) {
                ...
                let convertedFeeFormatted = amountFormatter.format(
    ...
    func buildModel(
        ...
        isRefund: LoadableModelItem<Bool>
    ) async -> SendConfirmationModel {
        ...
        case let .token(token, amount):
            ...
            case .ton:
                ...
                if let rates = rates.tonRates.first(where: { $0.currency == currency }) {
                    ...
                    formattedConvertedAmount = amountFormatter.format(
                        amount: converted.amount,
                        fractionDigits: converted.fractionLength,
                        accessory: .currency(currency)
                    )
            ...
            case let .jetton(jettonItem):
                ...
                formattedAmount = amountFormatter.format(
                    ...
                    accessory: jettonItem.jettonInfo.symbol.flatMap { .symbol($0) } ?? .none
                )
            }
        case let .nft(nft):
