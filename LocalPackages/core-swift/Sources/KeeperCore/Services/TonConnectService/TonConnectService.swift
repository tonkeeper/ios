...
public protocol TonConnectService {
    ...
    func createEmulateRequestBoc(
        wallet: Wallet,
        seqno: UInt64,
        timeout: UInt64,
        parameters: SendTransactionParam
    ) async throws -> String
    func createConfirmTransactionBoc(
        wallet: Wallet,
        seqno: UInt64,
        timeout: UInt64,
        parameters: SendTransactionParam,
        signClosure: (TransferData) async throws -> String
    ) async throws -> String
...
private extension TonConnectServiceImplementation {
    func rebuildJettonPayloads(wallet: Wallet, messages: [SendTransactionParam.Message]) async throws -> [SendTransactionParam.Message] {
        ...
        for message in messages {
            ...
            try rebuildedMessages.append(SendTransactionParam.Message(
                address: message.address,
                amount: message.amount,
                stateInit: jettonPayload.stateInit != nil ? jettonPayload.stateInit?.toBoc().base64EncodedString() : message.stateInit,
    ...
    func createRequestTransactionBoc(
        wallet: Wallet,
        seqno: UInt64,
        timeout: UInt64,
        parameters: SendTransactionParam,
