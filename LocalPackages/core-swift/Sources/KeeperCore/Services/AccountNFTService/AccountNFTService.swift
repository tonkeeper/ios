...
public protocol AccountNFTService {
    ...
    func loadAccountsNfts(
        wallets: [Wallet],
        collectionAddress: Address?,
        limit: Int,
        offset: Int,
        isIndirectOwnership: Bool
    ) async throws -> [AccountNfts]
    func saveAccountNfts(
        wallet: Wallet,
        nfts: [NFT]
    ) throws
}
...
final class AccountNFTServiceImplementation: AccountNFTService {
    ...
    func loadAccountsNfts(
        ...
        return await withTaskGroup(of: AccountNfts.self, returning: [AccountNfts].self) { [weak self] taskGroup in
