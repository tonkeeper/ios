...
public struct TonConnectSessionCrypto {
    ...
    public init(sessionId: String? = nil) throws {
        let keyPair = try TweetNacl.NaclBox.keyPair()
        self.keyPair = KeyPair(
            publicKey: .init(data: keyPair.publicKey),
            privateKey: .init(data: keyPair.secretKey)
        )
        self.sessionId = sessionId ?? keyPair.publicKey.hexString()
    }

    public init(sessionId: String? = nil, privateKey: PrivateKey) throws {
    ...
    public func decrypt(message: Data, senderPublicKey: Data) throws -> Data {
        ...
        return try TweetNacl.NaclBox.open(
