    ...
    struct LedgerDevice: Codable, Equatable, CellCodable, Hashable {
        ...
        public static func loadFrom(slice: TonSwift.Slice) throws -> Wallet.LedgerDevice {
            return try slice.tryLoad { s in
                let deviceIdCount = try Int(s.loadUint(bits: .deviceIdCountLength))
                ...
                let deviceModelCount = try Int(s.loadUint(bits: .deviceModelCountLength))
                ...
                let accountIndex = try Int16(s.loadInt(bits: .accountIndexLength))
