...
protocol NativeSwapAPI {
    ...
    func subscribeToSwapConfirmation(data: SwapConfirmationData, network: Network) -> AsyncThrowingStream<SwapConfirmation, Error>
}

final class NativeSwapAPIImplementation: NativeSwapAPI {
    private enum NativeSwapAPIError: Error, Decodable {
            ...
            let task = Task { [weak self] in
                ...
                do {
                    ...
                    guard let requestURL = urlComponents?.url else {
                        ...
                        streamSession.invalidateAndCancel()
                        continuation.finish(throwing: NativeSwapAPIError.incorrectHost(url.absoluteString))
                        return
                    }
                    ...
                    for try await line in bytes.lines {
                        ...
                        if jsonString.contains("\"error\":") {
                            ...
                            streamSession.invalidateAndCancel()
                            continuation.finish(throwing: NativeSwapAPIError.serverError)
                            return
                        }
                    ...
                    logger.e("SSE stream error: \(error.localizedDescription)")
                    streamSession.invalidateAndCancel()
