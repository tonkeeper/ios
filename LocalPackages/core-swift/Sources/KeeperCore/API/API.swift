...
extension API {
    ...
    func getPoolInfo(poolAddress: Address) async throws -> StackingPoolInfo {
        let request = try await createRequest {
            StakingAPI.getStakingPoolInfoWithRequestBuilder(accountId: poolAddress.toRaw())
        }
        let response = try await performRequest(request: request).body
        return try StackingPoolInfo(accountStakingInfo: response.pool, implementations: [response.pool.implementation.rawValue: response.implementation])
    }
}
