...
extension AccountEvent {
    init(accountEvent: TonAPI.AccountEvent) throws {
        ...
        self.progress = Double(accountEvent.progress)
        ...
        self.actions = accountEvent.actions.compactMap { action -> AccountEventAction? in
            do {
                ...
                if let tonTransfer = action.tonTransfer {
                    actionType = try .tonTransfer(.init(tonTransfer: tonTransfer))
                } else if let jettonTransfer = action.jettonTransfer {
                    actionType = try .jettonTransfer(.init(jettonTransfer: jettonTransfer))
                } else if let contractDeploy = action.contractDeploy {
                    actionType = try .contractDeploy(.init(contractDeploy: contractDeploy))
                } else if let nftItemTransfer = action.nftItemTransfer {
                    actionType = try .nftItemTransfer(.init(nftItemTransfer: nftItemTransfer))
                } else if let subscribe = action.subscribe {
                    actionType = try .subscribe(.init(subscription: subscribe))
                } else if let unsubscribe = action.unSubscribe {
                    actionType = try .unsubscribe(.init(unsubscription: unsubscribe))
                } else if let auctionBid = action.auctionBid {
                    actionType = try .auctionBid(.init(auctionBid: auctionBid))
                } else if let nftPurchase = action.nftPurchase {
                    actionType = try .nftPurchase(.init(nftPurchase: nftPurchase))
                } else if let purchase = action.purchase {
                    actionType = try .purchase(.init(purchase: purchase))
                } else if let depositStake = action.depositStake {
                    actionType = try .depositStake(.init(depositStake: depositStake))
                } else if let withdrawStake = action.withdrawStake {
                    actionType = try .withdrawStake(.init(withdrawStake: withdrawStake))
                } else if let withdrawStakeRequest = action.withdrawStakeRequest {
                    actionType = try .withdrawStakeRequest(.init(withdrawStakeRequest: withdrawStakeRequest))
                } else if let jettonSwap = action.jettonSwap {
                    actionType = try .jettonSwap(.init(jettonSwap: jettonSwap))
                } else if let jettonMint = action.jettonMint {
                    actionType = try .jettonMint(.init(jettonMint: jettonMint))
                ...
                return try AccountEventAction(type: actionType, status: status, preview: .init(simplePreview: action.simplePreview))
            } catch {
                return nil
            }
        }
...
extension AccountEventAction.Subscription {
    init(subscription: TonAPI.SubscriptionAction) throws {
        ...
        self.amount = Int64(subscription.price.value) ?? 0
...
extension AccountEventAction.Purchase {
    init(purchase: TonAPI.PurchaseAction) throws {
        self.source = try WalletAccount(accountAddress: purchase.source)
        self.destination = try WalletAccount(accountAddress: purchase.destination)
        self.invoiceId = purchase.invoiceId
        self.amount = AccountEventAction
            .TokenAmount(
                value: purchase.amount.value,
                tokenName: purchase.amount.tokenName,
                currencyType: purchase.amount.currencyType.rawValue,
                decimals: purchase.amount.decimals,
                verification: purchase.amount.verification.rawValue,
                image: purchase.amount.image,
                jetton: purchase.amount.jetton
            )
        self.metadata = AccountEventAction.Metadata(encryptedBinary: purchase.metadata.encryptedBinary)
    }
}
