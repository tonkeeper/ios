...
private extension TonConnectEventsStore {
    ...
    func handleEvent(_ tonConnectEvent: TonConnectEvent) {
        ...
        for walletApps in walletsApps {
            guard let app = walletApps.value.apps.first(where: { $0.clientId == tonConnectEvent.from }) else { continue }
            ...
            do {
                ...
                Log.e("Log: Failed to handle ton connect event \(tonConnectEvent)", extraInfo: [
