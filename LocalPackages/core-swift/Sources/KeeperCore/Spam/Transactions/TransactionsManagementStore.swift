...
public extension TransactionsManagement {
  final class Store {
    ...
    private func updateTxState(_ txID: txID,
                               _ transactionState: TransactionState) async {
    ...
    private func updateTxState(_ txID: txID,
      ...
      queue.async { [weak self] in
        ...
        observers.forEach {
          $0.observer?.didGetTransactionsManagementStore(
            self,
            event: .updateState(txID: txID,
