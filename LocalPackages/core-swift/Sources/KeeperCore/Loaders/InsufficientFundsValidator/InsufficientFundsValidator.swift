...
public protocol InsufficientFundsValidator: AnyObject {
    ...
    func validateEmulationResultIfNeeded(_ emulation: SignRawEmulation, wallet: Wallet, numOfInternals: Int?) throws
}
...
final class InsufficientFundsValidatorImplementation: InsufficientFundsValidator {
                ...
                if case let .extra(extra) = emulationModel.extraState {
                    ...
                    case let .default(extraAmount):
                        if !isRefund, formattedTonBalance < extraAmount {
                            throw InsufficientFundsError.blockchainFee(wallet: wallet, balance: formattedTonBalance, amount: extraAmount)
                        }
                    case .battery:
                        break
                    case let .gasless(_, extraAmount):
                        ...
                        if requiredAmount > balance {
                            throw InsufficientFundsError.blockchainFee(wallet: wallet, balance: balance, amount: requiredAmount)
                        }
                    }
                }
            case .nft:
    ...
    func validateEmulationResultIfNeeded(_ emulation: SignRawEmulation, wallet: Wallet, numOfInternals: Int? = nil) throws {
