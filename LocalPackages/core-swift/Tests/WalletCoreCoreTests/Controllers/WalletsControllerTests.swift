...
final class WalletsControllerTests: XCTestCase {
    ...
    func test_has_no_wallets_if_no_keeper_info() {
    ...
    func test_has_wallets_if_has_regular_wallet_and_mnemonic_and_external_wallet() throws {
        ...
        let publicKey = try TonSwift.PublicKey(data: XCTUnwrap(String(repeating: "2", count: 32).data(using: .utf8)))
    ...
    func test_add_regular_wallet_updates_keeper_info_and_save_mnemonic() throws {
        ...
        let wallet = try XCTUnwrap(mockKeeperInfoRepository.keeperInfo?.wallets[0])
        XCTAssertEqual(try mockWalletMnemonicRepository.mnemonics[wallet.identity.id()], mnemonic)
    }
    ...
    func test_add_external_wallet_notify_observers() throws {
        ...
        let publicKey1 = try TonSwift.PublicKey(data: XCTUnwrap(String(repeating: "2", count: 32).data(using: .utf8)))
        try walletsController.addExternalWallet(with: publicKey1)
        XCTAssertEqual(observer.addedWallets.count, 1)

        let publicKey2 = try TonSwift.PublicKey(data: XCTUnwrap(String(repeating: "3", count: 32).data(using: .utf8)))
    ...
    func test_get_private_key_for_regular_wallet() throws {
        let mnemonic1 = try Mnemonic(mnemonicWords: TonSwift.Mnemonic.mnemonicNew())
        let keyPair = try MnemonicLegacy.anyMnemonicToPrivateKey(mnemonicArray: mnemonic1.mnemonicWords)
        try walletsController.addWallet(with: mnemonic1)

        let privateKey = try walletsController.getWalletPrivateKey(walletsController.activeWallet)
        XCTAssertEqual(privateKey, keyPair.privateKey)
    }

    func test_get_private_key_for_external_wallet_throws_error() throws {
        let publicKey = try TonSwift.PublicKey(data: XCTUnwrap(String(repeating: "3", count: 32).data(using: .utf8)))
        try walletsController.addExternalWallet(with: publicKey)

        XCTAssertThrowsError(try walletsController.getWalletPrivateKey(walletsController.activeWallet))
    }
}
...
private final class Observer: WalletProviderObserver {
    ...
    func walletProvider(
        _ walletProvider: WalletProvider,
